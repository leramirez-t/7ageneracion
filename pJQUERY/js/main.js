...
const getKodersFromDb = () => {
    kodersArray = [];
    $.ajax({
        url:"https://javascript-ajax-d0ce6.firebaseio.com/eduardo/koders/.json",
        method:"GET",
        success:(response)=>{
...
const deleteKoderInDb = () => {
    ...
    $.ajax({
        url:`https://javascript-ajax-d0ce6.firebaseio.com/eduardo/koders/${koderKey}.json`,
        method:"DELETE",
        success:(response)=>{
...
const postKoderInDb = (koderObject) => {
    let koderKey = $(event.target).data("koder-index")
    $.ajax({
        url:`https://javascript-ajax-d0ce6.firebaseio.com/eduardo/koders/.json`,
        method:"POST",
